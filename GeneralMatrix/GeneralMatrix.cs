...
namespace DotNetMatrix
{
    ...
    public class GeneralMatrix : System.ICloneable, System.Runtime.Serialization.ISerializable, System.IDisposable
    {
        ...
        public static List<double> operator *(GeneralMatrix a, List<double> b)
        {
            if (a.ColumnDimension != b.Count)
                throw new ArgumentException("BadRank");

            var t = new List<double>(a.RowDimension);
            for (int i = 0; i < a.RowDimension; i++)
            {
                double s = 0;
                for(int j=0;j<a.ColumnDimension;j++)
                {
                    s += a.GetElement(i, j)*b[j];
                }
                t[i] = s;
            }
            return t;
