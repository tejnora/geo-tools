    ...
    public class VfkMultiLine : VfkDrawObjectBase, IDrawObject
    {
        ...
        public VfkMultiLine(VFKMain owner, VFKHPTableItem item, bool fromModifyItems)
        {
            ...
            foreach (VFKSBPTableItem table in sbpPoint)
            {
                ...
                var segment = new LineSegment(Owner.getSouradniceBodu(prevTable.BP_ID), Owner.getSouradniceBodu(table.BP_ID),
                    prevTable, table);
                if (!segment.GetIsValid())
                {
                    throw new VfkMultiLineException(string.Format("Vadny soubor. - Nenalezene body u HP line:{0}", Item.ID));
                }
        ...
        public VfkMultiLine(VFKMain owner, VFKZVBTableItem zvbItem, bool fromModifyItems)
        {
            ...
            foreach (VFKSBPTableItem table in sbdPoint)
            {
                ...
                if (!segment.GetIsValid())
                {
                    throw new VfkMultiLineException(string.Format("Vadny soubor. - Nenalezene body u ZVB line:{0}", Item.ID));
                }
                MultiLineSegments.Add(segment);
                prevTable = table;
            }
        ...
        public VfkMultiLine(VFKMain owner, VFKDPMTableItem dpmItem, bool fromModifyItems)
        {
            ...
            if (VfkElement.DpmType == DpmType.HCHU || VfkElement.DpmType == DpmType.HOCHP)
            {
                ...
                foreach (var table in sbmPoints)
                {
                    ...
                    if (!segment.GetIsValid())
                    {
                        throw new VfkMultiLineException(string.Format("Vadny soubor. - Nenalezene body u DPM line:{0}", Item.ID));
                    }
                    MultiLineSegments.Add(segment);
                    prevTable = table;
                }
        ...
        #endregion
        #region Lines
        private List<LineSegment> MultiLineSegments
        {
            get;
            set;
        }
        ...
        public override void InitializeFromModel(UnitPoint point, ICanvasLayer layer, ISnapPoint snap)
        {
            ...
            if (Type == VfkMultiLineType.Op || Type == VfkMultiLineType.Hbpej ||
                (Type == VfkMultiLineType.DPM && (VfkElement.DpmType == DpmType.HCHU || VfkElement.DpmType == DpmType.HOCHP)))
            {
                MultiLineSegments = new List<LineSegment>();
            ...
            if (snap == null) return;
            MultiLineSegments = new List<LineSegment>();
            MultiLineSegments.Add(new LineSegment(((IVFKTool)snap.Owner).getActivePoint(0), ((IVFKTool)snap.Owner).getActivePoint(0), null, null));
            SetVfkElement(Singletons.VFKElements.SelectedSubGroup.SelectedElement);
            Selected = true;
        }
        ...
        {
            return string.Format("VfkMultiLine, Typ={0}, Pocet Bodu={1}, Typppd_kod={2}", VfkElement, MultiLineSegments.Count, VfkElement.TYPPPD_KOD);
        }
