...
namespace CAD.UITools
{
    public partial class VfkToolBar : GeoCadToolBar
    {
        ...
        {
            get => ToolBarManager.Document != null && ToolBarManager.Document.DataModel.ViewPointEnable;
            set
            {
        ...
        void OnVfkLayerManger(object sender, EventArgs args)
        {
            if (ToolBarManager.Document.DataModel.ShowVfkLayerManager())
                ToolBarManager.Document.CanvasCommand.InvalidateAll();
        }

        void OnReloadTool(object sender, SelectionChangedEventArgs e)
        {
